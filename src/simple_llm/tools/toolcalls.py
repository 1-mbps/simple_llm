...
def capitalize_type_values(d: dict) -> dict:
    """
    Recursively capitalizes all values associated with the key "type" in a nested dictionary.
    
    :param d: Dictionary to process
    :return: Dictionary with capitalized "type" values
    """
    if isinstance(d, dict):
        return {k: (v.upper() if k == "type" and isinstance(v, str) else capitalize_type_values(v)) for k, v in d.items()}
    elif isinstance(d, list):
        return [capitalize_type_values(item) for item in d]
    return d